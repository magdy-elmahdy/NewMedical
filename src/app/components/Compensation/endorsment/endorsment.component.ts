...
export class EndorsmentComponent {
  ...
  PumpArr:any
  netPremiumVal:any
  constructor(private _ToastrService:ToastrService ,private _AdminService:AdminService,private _PolicyService:PolicyService,public _DatePipe:DatePipe,private _Router:Router,private _ActivatedRoute:ActivatedRoute ,private _AppendixService:AppendixService){
...
TechnicalForm:FormGroup= new FormGroup({
  ...
  this._AppendixService.AddPolicyAppendix(this.formData).subscribe( async (res:any)=>{
    ...
    if(this.AppendType=='Add'){
      ...
      await this._Router.navigate(['/EndorsmentPrem/'+this.PolicyId+'/'+res.endorsement.id])
    }else if(this.AppendType=='Remove'){
      ...
      await this._Router.navigate(['/CancleUpdate/'+this.PolicyId+'/'+res.endorsement.id])
      Swal.fire({title:'Endorsment Pumb Successfully Done' ,icon:'success',timer:4000,timerProgressBar:true})
    }
